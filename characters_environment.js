...
function starthefreakingamenoworyouknow(){
  GameStatus="start";
  document.getElementById("status").innerHTML="game is loading okko!!"
}
/*====================================
=            Game Status             =
====================================*/
...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("i said that you rotate your nose ok if you will not do it you are a hell thatnks for not letting the person say something ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
...
function checkStatus(character) {
  ...
  if (character.live == false && character.liveNumber == 0) {
    gameConfig.status = "gameover";mario_game.play()
